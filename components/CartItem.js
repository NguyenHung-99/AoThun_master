        ...
        <tr>
           <td style={{width: '100px', overflow: 'hidden'}}>
            ...
            <td className="align-middle" style={{minWidth: '150px'}}>
                ...
                onClick={ () => dispatch(decrease(cart, item._id, item.sizeSelection)) } 
                disabled={item.quantity === 1 ? true : false} > - </button>

                <span className="px-3">{item.quantity}</span>

                <button className="btn btn-outline-secondary"
                onClick={ () => dispatch(increase(cart, item._id, item.sizeSelection)) }
                disabled={item.quantity === item.size.filter(it => it.Size === item.sizeSelection)[0].InStock_Size ? true : false} > + </button>
            ...
            <td className="align-middle" style={{minWidth: '50px', cursor: 'pointer'}}>
                ...
                onClick={() => dispatch({
                    type: 'ADD_MODAL',
                    payload: [{ data: cart, id: item._id, sizeSelection: item.sizeSelection, title: item.title, type: 'ADD_CART' }]
                })} ></i>
            </td>
        </tr>
    )
